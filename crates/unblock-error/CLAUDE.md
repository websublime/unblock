...
Also the two SHARED untrusted-input helpers (D43), homed here because the JSON ingestion boundaries
...
payload may echo. Since D49 `clip` has a third production consumer, `unblock-mcp`'s startup-failure
render. Zero new dependencies: this crate stays the deepest leaf.
