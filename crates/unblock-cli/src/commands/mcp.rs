//! `unblock mcp` (FR-20, D27/AD-4) — the PRIMARY runtime command.
//!
//! Opens a `WorkspaceContext`, installs the FR-17 shutdown handle, opens the `Session` wired to the
//! handle's flag, and runs the LIVE 2-arg `unblock_mcp::run_mcp_server(Arc<Session>, McpServerOptions)` (transport
//! internal `stdio()`). On EOF / first signal the `CancellationToken` cancels → `run_mcp_server` returns `Ok`
...
pub async fn run(_args: &McpArgs, overrides: &CliOverrides) -> Result<Option<u8>, CliError> {
    ...
    run_mcp_server(Arc::clone(&session), opts)
        .await
        .context(McpSnafu)?;
    ...
    // fall through to the normal `ExitCode` return: `run_mcp_server` binds `tokio::io::stdin()`, whose read is
