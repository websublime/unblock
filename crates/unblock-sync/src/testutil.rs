...
impl Storage for FakeStorage {
    ...
    /// `Ok(Vec::new())` would be worse still: a double that always answers "this workspace is clean"
    /// makes every dangling assertion in the crate vacuously green, which is the exact failure mode
    /// this file's header exists to forbid. The edge set is each row's own `dependencies`; an edge is
    /// dangling iff no row carries its target — EXISTENCE alone, never status, so a closed or
    /// tombstoned target counts as existing — and `external:` targets are excluded through the ONE
    /// shared L0 predicate. Ordered `(issue_id, dep_type, depends_on_id)`, like the real read's SQL.
