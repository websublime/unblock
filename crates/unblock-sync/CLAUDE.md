...
A duplicated JSON key on a `bd` line is **REJECTED, not last-win collapsed** (D43) — the scan runs
on the raw line bytes, upstream of the `Value` that would have already merged it.
