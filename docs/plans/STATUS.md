...
| T0.3 | `unblock-model` (types, hash, sync-eq, validation) | T0.2 | ◐ | Branch `t0.3-unblock-model`. Spec-first PART 1 landed (spine §1.8 frozen 17-field hash padding tail documented Q4=KEEP; spine §1.1–§1.4 illustrative `#[serde(untagged)]` removed → hand-rolled `Serialize`/`Deserialize`/`JsonSchema`; crate plan: `src/id.rs` row, full validator rule set + empty-title-as-`FieldError`, Q4/Q5 resolved, `sync_equals` id-first + estimated_minutes, `into_tombstone` contract, `[features] toon = []`). Impl in progress. |
