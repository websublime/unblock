...
| T0.4 | `unblock-error` (snafu taxonomy, exit codes) | T0.2 | ◐ | branch `t0.4-unblock-error`; spec-first PART 1 (spine §2 D-E1 `FieldError`/`ValidationFailed`, 35-variant reconcile, OQ-1/2/4/5 + model Q2 resolved) + crate impl. **Verify gate = PASS_WITH_CHANGES, applied:** (1) sanitize `hint` at the L0 chokepoint (with_hint/from_coded/From), spine §2.4 covers message+hint+context-render note; (2) bound `find_similar_ids` input (`MAX_SUGGESTION_INPUT_CHARS=256`) + two-row Levenshtein. `unblock-error` fuzz target (`fuzz_sanitize`) **deferred to T0.7** (see that row); proptest covers the invariants now. **Pending PR/merge.** |
| T0.5 | `unblock-storage`: `Storage` trait | T0.3, T0.4 | ☐ | |
| T0.6 | `unblock-storage`: libsql impl (WAL, busy_timeout) | T0.5 | ☐ | |
| T0.7 | Storage contract test suite (NFR-16) | T0.6 | ☐ | also wires the nested `unblock-fuzz` cargo-fuzz targets (nightly, `exclude`d from stable workspace — NFR-12), **incl. T0.4's deferred `fuzz_sanitize`** over `unblock_error::sanitize_message` |
