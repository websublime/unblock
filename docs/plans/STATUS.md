...
| T0.6 | `unblock-storage`: libsql impl (WAL, busy_timeout) | T0.5 | ◐ | **IN PROGRESS** (branch `t0.6-libsql-impl`). PART 1 (spec-first, this commit): spine §3.2.1 method-semantics block (create=no dedup · update=per-field events + empty-diff full skip · delete=`into_tombstone`+`Event(Deleted)` only from non-terminal · claim=assignee-only guard · ready=`idx_issues_ready`, wisp DROPPED · blocked=`status NOT IN closed/tombstone` incl. in_progress, two live passes · search=`instr(lower)` over title+desc+id) + the EventType-per-mutation oracle table; spine §3.3 (busy_timeout=5000 native, BEGIN IMMEDIATE, wal_autocheckpoint=0+manual, OQ-5 = write conn + separate read conn / shared-cache `:memory:`, `From<libsql::Error>` busy/locked map); crate plan §3.3 rows (38-col schema, CASCADE/no-FK, CURRENT_SCHEMA_VERSION=1, UPSERT ids, no-dedup crud, assignee-only claim, instr search, two-step live blocked, oldest-first events) + OQ-1/OQ-2/OQ-4/OQ-5 RESOLVED. PART 2 (libsql backend) lands next commit. |
