...
- Top-10 statically-linked tree-sitter grammars: Rust, TypeScript, JavaScript, Python, Go, Java, C, C++, Ruby, PHP — exposed via Cargo feature flags.
...
### Parser / grammars
- **L4.** `tree-sitter` Rust crate + `tree-sitter-<lang>` upstream crates **statically linked** via Cargo feature flags. Top-10 grammars default-enabled.
...
### Languages
- **L10.** v1.0.0 Top-10: Rust, TypeScript, JavaScript, Python, Go, Java, C, C++, Ruby, PHP. C# / Swift / Dart deferred to v1.0.x via follow-up bead.
...
- **L20.** `find-symbol` p99 < **10 ms** warm (medium repo, corpus per research R8); `outline` p99 < **20 ms** warm.
...
| `languages` | `{tool, languages:[{language, file_count, symbol_count}]}` |
...
| `tree-sitter-rust`, `…-typescript`, `…-javascript`, `…-python`, `…-go`, `…-java`, `…-c`, `…-cpp`, `…-ruby`, `…-php` | Static-linked grammars (Top-10) | New (10 crates) |
...
- **R-CLI-4 — `build.rs` mechanics.** Verify upstream `tree-sitter-<lang>` crates compile across Linux / macOS / Windows **without** requiring the user's machine to have a `cc` toolchain (they should ship a generated `parser.c`). Document any platform that fails.
- **R-CLI-5 — R10 reframe (ROI harness).** Re-baseline the 3 aspirational flows (A/B/C from prior R10) under the new transport: indexer arm uses `Bash("unblock-code …")`. Sonnet via the Anthropic API with a Claude-Code-like system prompt. The 2.0× ratio is now a **SOFT** target (per L22 / S5) — measured and published, follow-up bead if missed, no ship-block. Absolute numbers are reset.
...
- `Language` enum + Cargo feature flags (`lang-rust`, `lang-typescript`, …) — Top-10 default-enabled.
- Loader registry exposes a `tree_sitter::Language` per active language at runtime.
- `unblock-code languages` returns `{tool, languages: [{language, file_count: 0, symbol_count: 0}]}` (counts populated once Epic 03.5 lands).
- Acceptance: `cargo install unblock-code --no-default-features --features lang-rust` produces a working binary that exposes only Rust.
...
- Add 4 hand-written extension queries (field / property / import / export) per prior Resolution Q5.
...
- ROI harness per L22 (SOFT gate S5): 3 representative agent flows × N=10 runs each, baseline = `Glob/Grep/Read`, indexer = `Bash("unblock-code …")`. Sonnet via Anthropic API. Numbers are measured and published as a release artifact; if median < 2.0× on any flow, open a `unblock:finding:risk` follow-up bead — do **not** block phase close. Harness must still land **before** the perf-tuning beads in 03.6 so the ratio is observable throughout the epic, even though it no longer gates release.
...
- **H2.** `unblock-code` binary builds with default features (Top-10 grammars) on Linux x86_64, macOS aarch64, Windows x86_64 — without requiring the user to have a `cc` toolchain (validated in research R-CLI-4).
- **H3.** Top-10 languages parse a mixed-language fixture repo without panic.
- **H4.** All 11 CLI commands return JSON envelopes that validate against the spec's schema fixtures.
- **H5.** `find-symbol` p99 < **10 ms** warm; `outline` p99 < **20 ms** warm — measured on the medium representative repo (corpus per research R8).
