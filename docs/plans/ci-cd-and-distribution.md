...
| `doc-lint` | `cargo xtask doc-lint` — **doc-corpus consistency lint** (see §2.1) over the fixed 19-file corpus; catches the D-id / FR-tier / command-token / stamp / cross-ref / doc-count / MCP-surface-count drift classes. | — | **M0 (T0.9)** |
...
- **(g) MCP surface-count coherence** — every documented count of the MCP **tools** / **resources** / **prompts** must equal the count the server actually advertises. The count is **DERIVED** (parsed, never a second hard-coded literal) from the FR-12 discovery builder `crates/unblock-mcp/src/resources/capabilities.rs` — the lint masks its comments+string literals and counts the tuple entries of `tool_descriptors()` / `resource_descriptors()` / `prompt_descriptors()` (currently **8 / 5 / 3**). This is the sole source file the lint reads outside the doc corpus; a missing/unparseable anchor or a zero count is a hard FAIL (never a vacuous pass). The builder is kept equal to the LIVE `rmcp` router by `crates/unblock-mcp/tests/contract_suite.rs::live_list_tools_equals_the_builder_eight`, so pinning the docs to the builder pins them, transitively, to the real server — this class is the machine guard that would have caught the D37 **7→8** tool-count drift that survived two doc cascades. A `<N> [consolidated ][MCP ](tools|resources|prompts)` literal FIRES when `<N>` disagrees with the matching count, EXCEPT: (i) the digit is a section-ordinal or an embedded id — preceded by `.`/`-`/`§` (spine `§5.4 Resources`, `RK-3 tool-count`, `12.2 MCP tool`), by a LETTER so the digit is part of an identifier token (`v1 tool count`, `L7 tool-registration`, `v1 prompts`), or on a `#` heading line — which is never a count; (ii) the noun resolves to its OWN count, so `5 resources` / `3 prompts` are checked against resources/prompts and never confused with the tool count; (iii) a **historical/superseded** occurrence is exempt — its line carries a transition marker (`7→8`, `superseded`, `amendment`, `used to`, `formerly`) OR it enumerates the pre-D37 seven-tool roster (`issue/claim/defer/query/dep/sync/diagnostics` with no `comment`). `create_bulk` is a discriminator **arm** of the `issue` tool, NOT a 9th tool: `8 tools` is correct, and a doc calling the surface `9 tools` (or reviving the D22 'stays 7' narrative un-annotated) is the drift this catches. **Discipline: to keep a non-current count in a doc, annotate it historical** (arrow / `superseded` / `amendment`); an un-annotated stale count fires — which is precisely how this class makes the invisible drift visible.
