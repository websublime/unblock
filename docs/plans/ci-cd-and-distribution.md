...
- **(a) D-id coherence** — each `D-id` (D1..D43) appears with the **same** version tag, packaging, and verification scheme everywhere it is referenced (would have caught D12-vs-D17 self-update drift). *(The D-set is PRD-§4-data-driven: the lint parses the defined ids from the `| **Dx** |` rows and resolves membership against that set — the range is documentation, not a hard-coded regex; adding D43 to PRD §4 extends the set with no lint code change — as D41, D42 and D43 each did.)*
