...
| `stress-integrity` | long-lived single-workspace stress + interleaved concurrent command-family integrity (create/update/close vs export/import vs read) — INTEGRITY-only (no corruption / no partial write / linearizable) at a modest corpus (NFR-5 stress/interleaving half). | NFR-5 | **DEFERRED → T3.4** (ships with the engine reliability gates + the `unblock-sync/src/atomic.rs` fault-injection seam) |
...
- **(a) D-id coherence** — each `D-id` (D1..D30) appears with the **same** version tag, packaging, and verification scheme everywhere it is referenced (would have caught D12-vs-D17 self-update drift). *(The D-set is PRD-§4-data-driven: the lint parses the defined ids from the `| **Dx** |` rows and resolves membership against that set — the range is documentation, not a hard-coded regex; adding D30 to PRD §4 extends the set with no lint code change.)*
