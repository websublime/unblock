...
| `rate-limit` | single-MCP-server rate-limit assertions (NFR-18 rate-limit half); a cheap, **independent** `-p unblock-mcp` leg (`crates/unblock-mcp/tests/rate_limit.rs`) | NFR-18 | **owned by / lands at T3.5 (P2)** |
