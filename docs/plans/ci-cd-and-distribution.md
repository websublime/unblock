...
| `scale` | 250k-issue corpus under the single-MCP-server topology (D14) | NFR-2 | **DEFERRED → T3.5** (the 250k corpus harness is built at T3.5) |
...
| `rate-limit` | single-MCP-server rate-limit assertions (NFR-18 rate-limit half). | NFR-18 | **DEFERRED → T3.5** (rides the perf/scale corpus) |
...
- **(a) D-id coherence** — each `D-id` (D1..D32) appears with the **same** version tag, packaging, and verification scheme everywhere it is referenced (would have caught D12-vs-D17 self-update drift). *(The D-set is PRD-§4-data-driven: the lint parses the defined ids from the `| **Dx** |` rows and resolves membership against that set — the range is documentation, not a hard-coded regex; adding D32 to PRD §4 extends the set with no lint code change.)*
