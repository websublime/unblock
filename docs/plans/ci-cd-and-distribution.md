...
| `no-network` | **workspace-wide no-network source-scan** — assert no networking symbols (`reqwest`, `hyper`, `std::net`, raw TLS, un-gated `axoupdater`) link into any crate except the whitelisted `self-update`-feature-gated `axoupdater` TLS path in `unblock-cli`; spot-checks the default-feature binary too. **The whitelist is a LIST and D51 makes it grow.** When REMOTE mode's client lands, `unblock-storage` gains an HTTP path behind the `remote` feature, so the scan needs a SECOND explicitly feature-gated entry or this required job fails for a reason unrelated to its own decision. Two sub-questions are open at the v1.3 lock and are named rather than answered — whether the client crate's own name joins `NETWORK_SYMBOLS` (it is not there today, so a module that names only that crate would pass the scan silently and the job's "unblock's ONLY network surface is `unblock update`" premise would go quietly false), and what the entry's path/symbol/gate triple is, mirroring the `UPDATER_PATH` + `UPDATER_EXEMPT_SYMBOL` + `assert_update_mod_gated` shape at `xtask/src/no_network.rs:41-65`, with the function itself at `:210`. | NFR-17 | **LANDS at T3.6** (workspace-wide source-scan; whitelists the `self-update` axoupdater TLS path). T3.1 already landed the crate-scoped static tripwire `tests/no_git_gate.rs` (no `Command::new("git")`/git crate/network symbol on the default build; the only network dep is behind `self-update`); T3.6 adds the workspace-wide CI `no-network` xtask job (scans all `crates/*/src` + `xtask/src`) + the default-feature binary spot-check. |
...
### 2.2 Targeted features vs `--all-features` (D15/D51/NFR-10 — the M0 gate must not link an HTTP client)

**The rule survives; its stated reason has been wrong since T0.6 and is corrected here rather than carried.** `--all-features` cannot resolve a libsql `remote` feature today, because `crates/unblock-storage/Cargo.toml` declares `remote = []` — an empty marker forwarding to nothing — and the root manifest pins `libsql` at `default-features = false, features = ["core"]`. There is no edge to resolve. What `--all-features` WILL pull once REMOTE mode lands is the storage crate's SQL-over-HTTP client and its `reqwest`/`rustls` surface (D51), which D15/NFR-10 keep **off the default path**. So the testkit clippy/test steps use **targeted features** — `-p unblock-storage --features testkit` — which is verified network-free (`testkit` pulls no deps). **Never `--all-features` in CI** until/unless remote mode is itself a tested target (v1.3). **A second, separate gap is recorded rather than closed here:** `deny.toml` sets `all-features = false` and `cargo audit` resolves the same default set, so the feature-ON leg is invisible to both required supply-chain jobs. The spike's clean audit was measured on a scratch crate, not on these jobs. Specifying that leg is a v1.3-lock obligation.

**One thing this section has never said and should.** `crates/unblock-cli/Cargo.toml` sets `default = ["self-update"]`, so `reqwest`/`hyper`/`rustls` are ALREADY in the default workspace tree via `axoupdater` (spine §6.7 acknowledges this; this document did not). "Keep TLS off the default build" has not been literally true at WORKSPACE scope since axoupdater landed. It is true of the default STORAGE path, which is what the rule actually protects.
...
- **(a) D-id coherence** — each `D-id` (D1..D51) appears with the **same** version tag, packaging, and verification scheme everywhere it is referenced (would have caught D12-vs-D17 self-update drift). *(The D-set is PRD-§4-data-driven: the lint parses the defined ids from the `| **Dx** |` rows and resolves membership against that set — the range is documentation, not a hard-coded regex; adding D51 to PRD §4 extends the set with no lint code change — as D41, D42, D43, D44, D45, D46, D47, D48, D49, D50 and D51 each did.)* **This string is one of the PROSE D-range bump sites** (the others are `CLAUDE.md`'s document map and the `xtask/src/doc_lint.rs` tokenizer comment, whose regex alternation moves with its prose) — **and the prose sites are only part of the cascade, which `docs/PROCESS.md` §3 states normatively as a LIST THAT CARRIES NO COUNT, all in the SAME commit** *(a derived count rotted here five times, the last of them into a number matching neither reading of its own enumeration; the enumeration IS the rule, and `scripts/checks/d50-pre-handshake-gate-claims.sh` pins every file the list names as carrying the live range, so the list cannot be off-by-one against itself — the reference is always the NEWEST script, because by the self-row rule a script never pins its own knob, so the moment the list grows the previous reference stops covering it)*: the prose sites PLUS the live-range knob of each shipped required check script (`scripts/checks/d44-create-deps-claims.sh` `RANGE_RE`; `scripts/checks/ub-lp9.25-dangling-blocker-claims.sh` `RANGE_RE` **and** `RANGE_ALT_RE`; since the D46 implementation commit, `scripts/checks/d46-schema-migration-claims.sh` `RANGE_RE` **and** `RANGE_ALT_RE`; since the D47 implementation commit, `scripts/checks/d47-envelope-id-claims.sh` `RANGE_RE` **and** `RANGE_ALT_RE`; since the D48 implementation commit, `scripts/checks/d48-stdout-channel-claims.sh` `RANGE_RE` **and** `RANGE_ALT_RE`; since the D49 implementation commit, `scripts/checks/d49-startup-failure-render-claims.sh` `RANGE_RE` **and** `RANGE_ALT_RE`; and, since the D50 implementation commit, `scripts/checks/d50-pre-handshake-gate-claims.sh` `RANGE_RE` **and** `RANGE_ALT_RE`, the newest of them and so the one the reference above names). **The prose sites are PINNED by EVERY one of those scripts**, whose knobs track the LIVE range and never a frozen historical one — so a bump that stops at the prose turns those required `doc-lint` steps red for reasons unrelated to their own decisions, and a bump that moves only one script's knob turns the others red. The commit that MINTS the id carries the edit at every file in that list which ALREADY EXISTS — the **SPEC** commit, not the implementation one, because a range bump is itself normative text; a script minted later carries the live range from its own first commit, as the D46 one did. **THIS FILE'S row is ROW-ANCHORED on the class-(a) statement above, not file-level, and the reason is a defect D45 actually hit:** a file-level token check on a document that also mentions the range in explanatory prose passes even when the NORMATIVE statement still carries the retired literal — the prose satisfies the check. So the row anchors on the class-(a) line, and this document quotes the LIVE range in exactly ONE place, that normative statement; every other reference to it here is by NAME ("the live range", "the `RANGE_RE` knob"), deliberately, so no prose occurrence can ever satisfy a pin on the normative one.
...
**Named sub-check (its D47 sibling — POSITIVE-ONLY, and for a different reason than the D46 one):** D47 (PRD §4 — the un-decodable envelope `id` class, tracked as `ub-cnv`) has executable teeth — the transport's `-32600` arm, the crate-private `crates/unblock-mcp/src/envelope_id.rs` predicate, the three-tier CD-7 harness and the duplex/stdio/pre-handshake suites — so a behavioural regression is a red `test`. **What no test can carry is its OWN survival, and one published artefact is invisible to every lint in this repo:** `docs/roadmap.html` sits OUTSIDE the 19-file doc-lint corpus, so a v1.0.1 card that lists a fix set missing a fix would ship unnoticed. `scripts/checks/d47-envelope-id-claims.sh` is therefore a REQUIRED-LANDING check, and it is **deliberately positive-only** — not because D47 retires no framing (it retires plenty, in `wire.rs`'s normative module doc, the crate plan's `NEVER replies` heading, the spine's two closed enumerations and NS4's docstring) but because a NEGATIVE sweep for retired wording is the same defect in disguise: a claim REWRAPPED across two lines is unfindable in principle, so the sweep goes green while the false sentence is still in the tree. Every row is instead a spelling-INDEPENDENT positive landing keyed on a durable identifier. It asserts: the predicate module and its `scan` entry point; the transport arm's compile-time reply constant; **the EXHAUSTIVE `JsonRpcMessage` match — pinned by the presence of the `Response(_)` arm, because a `_` wildcard would compile, pass every cell, and silently reopen the hole an rmcp bump adding a fifth variant would drive through**; the DECODED-key rule (`next_key::<String>`), whose violation is invisible to any hand-written test that does not happen to use the `\u`-escaped spelling; the recovered arm still passing `Some(id)`, since passing `None` there still answers, still recovers the connection and still passes every cell that does not correlate by id, while releasing no waiting peer at all; the shared corpus's `any(test, feature = "test-util")` gate, without which the in-lib cells compile away into a vacuous pass; the store-EFFECT oracle, the only assertion that kills a "rebuild it as a Request on the recovered id and deliver it" implementation; the disclosed `-32700` residual still named at the transport; **BOTH tracker ids in `.unblock/issues.jsonl` — `ub-cnv` and the `ub-788` the D47 row NAMES as its residual, so the top document of the hierarchy carries no dangling id**; the rendered roadmap's D47 bullet; and this gate's own two-sided wiring (SPECIFIED in this paragraph, RUNNING in the workflow), so a script that exists but is unwired fails on its own rows rather than passing silently. **It also carries the row no sibling has: `docs/PROCESS.md`'s count-free LIST naming this script** — nothing pinned that line before, which is precisely why the enumeration could rot silently, and this is the first cascade that would notice. **AND the ROW-ANCHORED half pins the LIVE D-range at EVERY file that §3 list enumerates** — the three prose sites plus the `RANGE_RE`/`RANGE_ALT_RE` knob of each shipped required script — with **its own knob deliberately un-rowed**, following the precedent its D46 sibling states: the newest script is the REFERENCE the others are compared against, and a self-row would be vacuous by construction. That is also why §2.1(a) above named THIS script rather than the D46 one, when D47 landed, as the thing that pins every file the list names, and names the newest sibling since, because the reference must always be the newest and a script never pins its own knob. **SEQUENCING, the same discipline all three siblings state:** the D-range knobs move with the SPEC commit (a range bump is normative text), while this script, its workflow step, and the two prose edits that ADD it to both enumerations belong to the IMPLEMENTATION commit — the one that actually mints the file — so no prose ever names a script that does not yet exist, and a spec-only tree that shipped it would turn the required `doc-lint` job red against itself. **THE CONTRACT KNOB IS THE INVERSE COUPLING AND IT DOES NOT MOVE HERE, stated affirmatively because an unstated "we didn't bump" is indistinguishable from an oversight:** D47 mints NO `ErrorCode` and bumps NO contract — `-32600` is `rmcp::model::ErrorCode(i32)`, a transport-layer type in another crate, so `unblock_error::ErrorCode::ALL` stays 36, `capabilities()`/`schema_bundle()` are byte-unchanged and **`unblock.mcp.v1.9` stands**. This script's `CONTRACT_RE` row therefore pins the version as UNMOVED, and a silent bump riding this decision goes red.

**Named sub-check (its D48 sibling — POSITIVE-ONLY, for the same reason as the D47 one plus one of its own):** D48 (PRD §4 — a subcommand whose stdout is a PROTOCOL channel reports its structured error on STDERR, tracked as `ub-og3`) has executable teeth — the injected-sink cells in `crates/unblock-cli/src/exit.rs`, the spawning `crates/unblock-cli/tests/mcp_stdout_channel.rs` suite, and the two INVERTED `mcp_lifecycle.rs` cells — so a behavioural regression is a red `test`. **But THREE of this decision's obligations are unreachable from any test at all**, which is the reason this gate exists rather than a stylistic preference: the SINGLE-CALLER invariant clause (2)'s blast-radius argument rests on (a second `into_exit` caller passing a literal `Reports` re-opens `ub-og3` with every unit and end-to-end cell still green); the FOUR residual ids clause (6) names and leaves OPEN (the first two, `ub-kp7` and `ub-b1a`, since CLOSED by D50 and by D49); and `docs/roadmap.html`, which sits OUTSIDE the 19-file doc-lint corpus, so a v1.0.1 card listing a fix set missing a fix would ship unnoticed. `scripts/checks/d48-stdout-channel-claims.sh` is therefore a REQUIRED-LANDING check, and it is **deliberately positive-only** — not because D48 retires no framing (every "structured output strictly on stdout" sentence gains a carve-out) but because a NEGATIVE sweep for retired wording is the same defect in disguise: a claim REWRAPPED across two lines is unfindable in principle, so the sweep goes green while the false sentence is still in the tree. Every row is instead a spelling-INDEPENDENT positive landing keyed on a durable identifier. It asserts: the two-valued classification TYPE (a `bool`, or a collapse to one variant, is what that row notices); **the classifier and its CONSUMER as a two-file co-occurrence — `stdout_role` in both `src/cli.rs` and `src/lib.rs`, because a classification produced and then DROPPED is a fix that does nothing. The CONSUMER half is ROW-ANCHORED on the production line and requires the `cli.command.stdout_role()` call, NOT the bare token: as a file-level token it was MEASURABLY satisfied by `lib.rs`'s own doc comment about `Command::stdout_role`, so deleting the production line and hard-coding a role at the call site compiled and left the gate silent — the exact state the row exists to catch, and the case the script's own two-rule header calls a Q row and not a P one**; the carve-out arm itself, anchored on the line that routes the `Protocol` case to the stderr sink; the sink-injected `into_exit_to` core, without which the stream CHOICE is unobservable in-process; the spawning regression file; the hardened stdout framing guard (a line that merely PARSES as JSON is not enough — the blob IS valid JSON, which is exactly how it passed for the whole life of the suite) and the `UNBLOCK_OUTPUT_FORMAT` scrub that stops a host shell making every frame-only assertion vacuous; **`ub-og3` AND all FOUR residual ids in `.unblock/issues.jsonl` — `ub-kp7`, `ub-b1a`, `ub-c5o`, `ub-5v5` — one row EACH, so a failure says WHICH id vanished and the top document of the hierarchy carries no dangling id**; the rendered roadmap's D48 bullet; this gate's own two-sided wiring (SPECIFIED in this paragraph, RUNNING in the workflow); and `docs/PROCESS.md`'s count-free LIST naming this script. **AND the ROW-ANCHORED half pins the LIVE D-range at EVERY file that §3 list enumerates** — the three prose sites plus the `RANGE_RE`/`RANGE_ALT_RE` knob of each shipped required script, now including the D47 one — with **its own knob deliberately un-rowed**, following the precedent its D46/D47 siblings state: the newest script is the REFERENCE the others are compared against, and a self-row would be vacuous by construction. **It also carries TWO STRUCTURAL checks no table row can express, because each is a RELATION rather than the presence of a string:** the single-caller count above, and that every `Command` variant is classified inside `stdout_role` — clause (2) makes the no-`_`-arm exhaustiveness NORMATIVE, but rustc only asks the question while no wildcard exists, and `Self::Mcp(_) => Protocol, _ => Reports` satisfies the classifier's own unit cell COMPLETELY (a design-Review finding, not a hypothetical). **SEQUENCING, the same discipline all four siblings state:** the D-range knobs move with the SPEC commit (a range bump is normative text), while this script, its workflow step, and the two prose edits that ADD it to both enumerations belong to the IMPLEMENTATION commit — the one that actually mints the file — so no required-STEP ENUMERATION ever lists a step that is not in the tree. (Ordinary prose may name a planned script, and the D48 PRD row does; a CI job naming a missing one fails the job.) **THE CONTRACT KNOB IS THE INVERSE COUPLING AND IT DOES NOT MOVE HERE, stated affirmatively because an unstated "we didn't bump" is indistinguishable from an oversight:** D48 mints NO `ErrorCode` (`ErrorCode::ALL` stays 36), moves no published byte and changes only which STREAM an already-formed document is written to, so `capabilities()`/`schema_bundle()` are byte-unchanged and **`unblock.mcp.v1.9` stands**. This script's `CONTRACT_RE` row therefore pins the version as UNMOVED, and a silent bump riding this decision goes red.

**Named sub-check (its D49 sibling — POSITIVE-ONLY, for the same reason as the D47/D48 ones plus one of its own):** D49 (PRD §4 — the `unblock mcp` startup-failure message DESCRIBES the refused first frame instead of quoting it, tracked as `ub-b1a`) has executable teeth — the cells behind the `test-util` seam in `crates/unblock-mcp/src/error.rs` render every arm the decision makes normative, and the four-shape cell compares whole messages byte for byte — so a behavioural regression is a red `test`. **But THREE of this decision's obligations are unreachable from any test at all**, which is why this gate exists rather than as a matter of taste: the MARKER FOLD in `crates/unblock-mcp/src/tools/bulk_markdown.rs`, where the constant and the hard-coded literal are EQUAL BYTES, so reverting `clip_header` to the literal leaves every cell green — including the shipped one that asserts that literal at the clipped header's end, which clause (5) deliberately does NOT rewrite to the constant, since an assertion comparing the constant with itself would pass for any value; the residual ids clause (6) names and leaves OPEN, THREE of them (the first, `ub-kp7`, since CLOSED by D50), of which this gate rows `ub-o8s` and `ub-wx3` while `ub-kp7` is already pinned by the D48 sibling's own `P9` row (`scripts/checks/d48-stdout-channel-claims.sh:131`), so the row absent here is coverage that exists elsewhere rather than an oversight; and `docs/roadmap.html`, which sits OUTSIDE the 19-file doc-lint corpus, so a v1.0.1 card listing a fix set missing a fix would ship unnoticed. `scripts/checks/d49-startup-failure-render-claims.sh` is therefore a REQUIRED-LANDING check, and it is **deliberately positive-only** — not because D49 retires no framing (the message no longer quotes the frame) but because a NEGATIVE sweep for retired wording is the same defect in disguise: a claim REWRAPPED across two lines is unfindable in principle, so the sweep goes green while the false sentence is still in the tree. Every row is instead a spelling-INDEPENDENT positive landing keyed on a durable identifier. It asserts: the ONE description function, without which the `Transport` display falls back to interpolating rmcp's own blob and the whole decision is undone in a single edit; the four-shape regression PIN, which clause (7) makes the pin precisely because an end-to-end cell went vacuous once D50 gated that class; and the `Cancelled` byte-identity cell, D38's measured line. **Those three rows are ROW-ANCHORED on the items' own `fn` declarations and never on a rendered message** — a bare token over the file is satisfied by a comment that still names a DELETED item, which the Verify gate MEASURED on this very file, while the messages are already asserted to the byte by the cells, so a gate that grepped them too would go red on a legitimate reword. Neither form tells a RUNNING cell from an `#[ignore]`d one; that is the required `test` job's business. **The same anchoring pins the two things a bare token cannot**: `unblock_error::clip` on the three production bindings that carry it, because both files name `clip` in their own prose and a file-level token would pass over a render that had dropped the call — the defect its D48 sibling's `Q15` row records having MEASURED; and `TRUNCATION_MARKER` on the `clip_header` line that appends it, **the only thing in the tree that can catch the fold's revert**, which clause (7) names as the one mutation no cell kills. It also asserts **`ub-b1a` AND the two residual ids it rows in `.unblock/issues.jsonl` — one row EACH, so a failure says WHICH id vanished and the top document of the hierarchy carries no dangling id**; the rendered roadmap's v1.0.1 card, anchored on that card's own D49 bullet because a bare token over the file is satisfied by any mention anywhere in it; and this gate's own three-sided wiring (SPECIFIED in this paragraph, RUNNING in the workflow, LISTED in `docs/PROCESS.md`'s count-free enumeration) — **the SPECIFIED one ROW-ANCHORED on this paragraph's own opening line**, because §2.1(a) above names the script too, so a bare token over this file stays green with this whole paragraph deleted. **AND it pins the LIVE D-range at EVERY file that §3 list enumerates** — the three prose sites, which cost FOUR rows because the `xtask/src/doc_lint.rs` tokenizer comment carries the prose range and the regex alternation on ONE physical line, plus the `RANGE_RE`/`RANGE_ALT_RE` knob of each shipped sibling script — with **its own knob deliberately un-rowed**, following the precedent its D46/D47/D48 siblings state: the newest script is the REFERENCE the others are compared against, and a self-row would be vacuous by construction. Its marginal rows are `d48`'s `RANGE_RE` and `RANGE_ALT_RE`, which nothing in the tree pinned until this script landed — the reason §2.1(a) above named THIS script as the reference until the D50 sibling below took that place. **It carries NO structural check, and the absence is stated rather than left to be noticed:** its D48 sibling has two because D48 rests on RELATIONS (a call-site count, an exhaustive classification), while D49 rests on renders its cells assert directly. **TWO ROWS ARE KNOWN-RED ON THE COMMIT THAT MINTS THIS SCRIPT, disclosed here rather than discovered in CI:** `ub-o8s` and `ub-wx3` reach the export only through the TRACK re-export, which `docs/PROCESS.md` §6 places after both gates on the same branch, so they go green in the same PR — whose head is what the required job evaluates. **SEQUENCING, the same discipline all five siblings state:** the D-range knobs move with the SPEC commit (a range bump is normative text), while this script, its workflow step, and the two prose edits that ADD it to both enumerations belong to the IMPLEMENTATION commit — the one that actually mints the file — so no required-STEP ENUMERATION ever lists a step that is not in the tree. **THE CONTRACT KNOB IS THE INVERSE COUPLING AND IT DOES NOT MOVE HERE, stated affirmatively because an unstated "we didn't bump" is indistinguishable from an oversight:** D49 mints NO `ErrorCode` (`ErrorCode::ALL` stays 36) and bounds the CONTENT of a message that appears in neither `capabilities()` nor `schema_bundle()`, so both are byte-unchanged and **`unblock.mcp.v1.9` stands**. This script's `CONTRACT_RE` row therefore pins the version as UNMOVED, and a silent bump riding this decision goes red.

**Named sub-check (its D50 sibling — POSITIVE-ONLY, for the same reason as the D47/D48/D49 ones plus one of its own):** D50 (PRD §4 — the pre-handshake frame gate, tracked as `ub-kp7`) has executable teeth, since the decorator's in-module cells and the repointed and inverted `mcp_lifecycle.rs` cells make a behavioural regression a red `test`. **But some of this decision's obligations sit outside the tests that carry its behaviour.** The residual ids clause (9) names are unreachable from any test at all, each one beside the work's own id, and a dangling one would leave the top document of the hierarchy citing an id the record does not carry. `docs/roadmap.html` is unreachable too, since it sits OUTSIDE the 19-file doc-lint corpus, so a v1.0.1 card listing a fix set missing a fix would ship unnoticed. **The COMPOSITION LINE is the row this gate is built around, and the claim about it is narrow.** No IN-MODULE cell of the gate reads the composition — deleting the layer at `crates/unblock-mcp/src/server.rs:468` leaves `cargo test -p unblock-mcp --lib pre_handshake` GREEN, MEASURED. The gate's own cells build the decorator directly, and the one cell that name filter also runs from outside the module (`crates/unblock-mcp/src/error.rs:528`) builds none. What goes red for that deletion is the stdio lifecycle suite in `crates/unblock-cli/tests/mcp_lifecycle.rs` and workspace clippy, which reports the unreached layer as dead code and fails under `-D warnings`. The row earns its place as the doc-lint catch, which runs in seconds where the stdio suite spawns a child per cell, and as a catch that survives a subtler deletion keeping the type constructed elsewhere, where clippy has nothing to report and that stdio suite is the other catch. `scripts/checks/d50-pre-handshake-gate-claims.sh` is therefore a REQUIRED-LANDING check, and it is **deliberately positive-only**, because a NEGATIVE sweep for the framing D50 retires is the same defect in disguise — a claim REWRAPPED across two lines is unfindable in principle, so the sweep goes green while the false sentence is still in the tree. Every row is instead a spelling-INDEPENDENT positive landing keyed on a durable identifier. **The P rows** are `ub-kp7` plus each residual id the D50 row names, in `.unblock/issues.jsonl`, **one row EACH and named rather than numbered**, because an "n of m" description rots at the next mint exactly as `d48`'s `P9` did. That export is the ONLY file the P table reads, because a bare token is a sound landing only over a file that carries no commentary about the thing, and **all three wiring files carry some — MEASURED, each with the mutation a bare token has to catch**, so the three self-wiring landings are row-anchored instead. **The Q rows** are the gate type's own declaration line, its `Transport` impl and the two crate-private functions — the receive-side classifier and the send-side latch predicate — each anchored on its own declaration, so a doc comment naming a deleted item fails the row instead of satisfying it; the composition line anchored on `VersionClampingTransport::new` and REQUIRING `PreHandshakeGateTransport::new`, so deleting the layer fails the row while the anchor survives; the EXHAUSTIVE four-variant `JsonRpcMessage` match pinned by the presence of its `Response(_)` arm, because a `_` wildcard compiles, passes every cell and reopens the hole an rmcp bump would drive through; the `send()` latch's `ServerResult::InitializeResult` pattern, which is what keeps the latch off any value the clamp above this layer can rewrite; the compile-time reply constant, required BY ITS TEXT because the cells compare against their own copy of it; the in-module cells — the gate's own, plus the composition cell in `crates/unblock-mcp/src/server.rs` that drives a real duplex through the server entry point — and the two `mcp_lifecycle.rs` cells, each on its own `fn` declaration; the new stdout-closing method in `crates/unblock-cli/tests/common/mod.rs`, without which the repointed cell provokes nothing; the `CONTRACT_RE` knob at `crates/unblock-mcp/src/options.rs` pinning `unblock.mcp.v1.9` UNMOVED; every shipped sibling's knob lines with **this script's own knob deliberately un-rowed**, following the precedent its D46/D47/D48/D49 siblings state, the newest script being the REFERENCE the others are compared against and a self-row being vacuous by construction; the three prose sites over FOUR rows, since the `xtask/src/doc_lint.rs` tokenizer comment carries the prose range and the regex alternation on ONE physical line; `docs/roadmap.html` anchored on the v1.0.1 card's own D50 bullet; this paragraph's own opening line as the SPECIFIED row, because ci-cd §2.1(a) above carries the filename too, so a bare token over this file stays green with this whole paragraph deleted; the gate RUNNING in `.github/workflows/ci.yml`, anchored on every line of that workflow naming the script and requiring the matched line to BE the run step, since a commented-out step still carries the filename; and `docs/PROCESS.md` §3's count-free LIST, anchored on this script's own entry there and requiring both knob names beside it, since §3 names the script twice and a bare token survives the entry's deletion; and the `P9` reason string of `scripts/checks/d48-stdout-channel-claims.sh`, which D50 clause (14) corrects, anchored on that row and requiring the corrected text, so a later edit restoring the retired wording goes red — the only executable pin clause (14) has. Its marginal rows are `d49`'s `RANGE_RE` and `RANGE_ALT_RE`, which nothing in the tree pinned until this script landed — the reason ci-cd §2.1(a) now names THIS script as the reference. **It carries NO structural check, and the absence is stated rather than left to be noticed.** Its D48 sibling has two because D48 rests on RELATIONS, a call-site count and an exhaustive classification, while D50 rests on a decorator that is either installed or not, which the composition row reads directly, and on dispositions its cells assert frame by frame. **NO ROW IS KNOWN-RED ON THE COMMIT THAT MINTS THIS SCRIPT, disclosed here rather than discovered in CI.** All seven ids the P table rows already have a record in the export, because the D47, D48 and D49 cascades each exported theirs; what the TRACK re-export changes is `ub-kp7`'s STATE, and these rows pin PRESENCE, so the re-export `docs/PROCESS.md` §6 places after both gates leaves every P row green. **SEQUENCING, the same discipline every sibling states.** The D-range knobs move with the SPEC commit, because a range bump is normative text, while this script, its workflow step, and the prose edits that ADD it to both enumerations belong to the IMPLEMENTATION commit — the one that actually mints the file — so no required-STEP ENUMERATION ever lists a step that is not in the tree. **THE CONTRACT KNOB IS THE INVERSE COUPLING AND IT DOES NOT MOVE HERE, stated affirmatively because an unstated "we didn't bump" is indistinguishable from an oversight.** D50 mints NO `ErrorCode` (`ErrorCode::ALL` stays 36) and answers with rmcp's own transport-level `-32600`, which appears in neither `capabilities()` nor `schema_bundle()`, so both are byte-unchanged and **`unblock.mcp.v1.9` stands**. This script's `CONTRACT_RE` row therefore pins the version as UNMOVED, and a silent bump riding this decision goes red.
