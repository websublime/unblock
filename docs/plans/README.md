...
| [`crates/unblock-mcp.md`](crates/unblock-mcp.md) | L7 (PRIMARY). rmcp 1.0 stdio server — 7-tool taxonomy + resources + prompts, schemars-validated under quotas, `contract_version`-stamped. Thin adapter over `Session`. |
...
| Crate | v1 (LOCKED) | v1.1 (LOCKED) | v1.2 (PROPOSED) | v1.3 (PROPOSED) |
|---|---|---|---|---|
...
- **Version coherence.** No v1 file depends on a v1.2/v1.3 feature: `remote` is consistently a non-default Cargo feature across storage/config/engine/health/mcp/fuzz; scale/1M and compaction are fenced to v1.3; the contention lab is correctly the M0 exit gate before any crate depends on storage.
