...
| [`crates/unblock-storage.md`](crates/unblock-storage.md) | L2. Backend-agnostic `Storage` trait + one backend-aware impl per product mode (D51) — libsql for LOCAL mode, SQL-over-HTTP for REMOTE mode at v1.3 (PROPOSED) (WAL + native busy_timeout non-spin, transactional audit, contention lab). Depends on model+error only. |
...
| [`crates/unblock-mcp.md`](crates/unblock-mcp.md) | L7 (PRIMARY). rmcp 1.7 stdio server — 8-tool taxonomy + resources + prompts, duplicate-key-scanned at the transport (D43), quota-checked and strictly deserialized, `contract_version`-stamped. Thin adapter over `Session`. |
...
|---|---|---|---|---|
| `unblock-model` | ● all v1 types, hash/sync-eq/validation | ● Comment/EpicStatus populated | ◐ conditional `remote_state.rs` (shape open) | ◐ `compaction.rs` helpers |
| `unblock-error` | ● ErrorCode/StructuredError/ModelError | ● additive hints/context | ◐ remote-mode ErrorCode variants | ◐ ClaimExpired/CompactionConflict |
...
| `unblock-storage` | ● trait + libsql impl + contention lab | ◐ label/comment/epic/config-table methods | ● `sql_over_http/` (feature `remote`) | ● archival, perf indexes, 1M gate |
| `unblock-sync` | ● export/import/bd/path/conflict/atomic | ◐ `audit.rs` (FR-22) | (—) | ◐ compaction round-trip |
| `unblock-health` | ● lite: level/file_state/doctor/paths | ● full taxonomy: anomaly/classify/audit/recovery | ◐ `remote_health.rs` (shape open) | ◐ `scale.rs` |
...
3. **Version discipline.** Implement only the version you are assigned. v1/v1.1 are LOCKED (restate the PRD); v1.2+ are PROPOSED direction — do not pull a v1.3-feature dependency into a v1 file (the `remote` feature must stay off the default build, D15/D51/NFR-10).
...
- **CF-H — [RESOLVED] Export scope (closed/tombstone inclusion) is assumed, not specified, and round-trip identity depends on it.** *Resolution: spine states export includes all non-ephemeral issues (incl. closed + tombstones) and excludes `ephemeral`; confirms sync Q3 default and makes the round-trip property well-defined.* **[AMENDED by D45 — normative text in spine §1.10; this line is a pointer, not the rule]** — the `ephemeral` exclusion is no longer unconditional: an `ephemeral`/`-wisp-` row that stands in a non-external dependency relation with a KEPT row, **in EITHER direction**, is pulled into the export by the blocker-closure walk. Dropping the row while keeping the edge produced a file whose blocker is absent; dropping the edge instead silently turned blocked work into ready work (measured). **The direction matters and an out-only reading is the defect the second design-review round caught:** the `parent-child` edge is stored on the CHILD, while the blocked-set query blocks the epic PARENT through it, so a kept epic whose only open child was `ephemeral` arrived READY. With the either-direction rule the round-trip property is STRENGTHENED, not weakened: blocked-before ⇒ blocked-after.
  ...
  *Fix:* Add to spine (§4.1 export or a new §1.8 note): **export includes all non-ephemeral issues incl. closed + tombstones, excludes `ephemeral`.** Confirms the sync Q3 default and makes the round-trip property well-defined. **[AMENDED by D45 — see the resolution line above: the `ephemeral` exclusion now yields to the blocker-closure walk, so an `ephemeral` row standing in a non-external dependency relation with a kept row, in EITHER direction, IS exported. Spine §1.10 is the normative statement.]**
...
- **CF-K — [RESOLVED via D17] Self-update ships in v1 via the `axoupdater` library (a dependency of `unblock-cli`), not a separate crate.** *Resolution: the `unblock update` command embeds `axoupdater`; each artifact is verified via the dist installer's SHA256 checksum before the swap (NFR-17, D17/D35); GitHub artifact attestations are publish-side provenance (`gh attestation verify`), not on the update path; behind the default-on `self-update` feature so `--no-default-features` drops the network surface. No `unblock-update` crate exists; PRD §8.1's 12-crate list is unchanged. Distribution + updater are generated by `dist` (see `ci-cd-and-distribution.md`).*
