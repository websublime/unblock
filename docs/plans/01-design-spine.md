...
**mcp (FR-20 / D27/AD-4).** Opens a `WorkspaceContext` via `open_with_storage_with_cli`, builds a `SessionConfig { jsonl_export: ctx.config.jsonl_export, import_on_open: false, remote: false }` (`import_on_open` MUST stay false in v1 — `true` returns `FeatureNotWired{"sync"}`, exit 1), installs the FR-17 shutdown handle, opens the `Session` + `with_shutdown_flag`, then calls the LIVE **2-arg** `unblock_mcp::run_mcp_server(Arc<Session>, McpServerOptions { cancel, quotas: Quotas::default(), instructions })` (§0.1 — transport is internal `stdio()`). On EOF/first signal the `CancellationToken` cancels → `run_mcp_server` returns `Ok` → `session.shutdown()` (drain the permit, clean libsql close). stdout carries ONLY MCP framing (logging is stderr-only, NFR-14). Single-MCP-server-per-workspace (D14).
