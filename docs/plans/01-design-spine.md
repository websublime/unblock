...
**`RenderError` follows this pattern too (D27/AF-4, T3.1 — additive).** `unblock-render` already computes an inherent `code()`; T3.1 adds `impl unblock_error::CodedError for RenderError { fn code(&self) -> ErrorCode { self.code() } }` (delegating to the inherent map — one error → one code, §6.5 unchanged) so the uniform `(&err).into()` L7 bridge covers it like every other per-crate enum. The 4th render variant `RenderError::UnknownFormat { name: String }` (added at T3.1 — `parse_format`'s unknown-name arm) maps to `ErrorCode::ValidationFailed`, the same family as `UnsupportedFormat`/`FieldUnknown`; the §2.3 exit table is UNCHANGED (no new ErrorCode). **`McpServerError` is the deliberate exception:** it does NOT impl `CodedError` — the cli `exit.rs` maps it EXPLICITLY (`Transport`/`RunLoop` → `ErrorCode::InternalError`, exit 1; an MCP-server run-loop/transport failure is an INTERNAL condition, not a user IoError) via `StructuredError::from_code(InternalError, err.to_string())` (which already routes through `sanitize_message`, and which since **D49** receives a LENGTH-BOUNDED `Display` — a rejected first frame renders as a structural summary of its kind, its method and its envelope id, each clipped through `unblock_error::clip`, with `params`, `result` and the error body never rendered) — **absent a recorded signal AND absent a pre-`initialize` disconnect**. D49 bounds only the CONTENT of the `message` member; the `InternalError` mapping, the exit-1 code, the D48 stderr channel and the `StructuredError` member set are byte-unchanged by it. When the FR-17 handle recorded a signal, the signal exit `128+signo` takes precedence over this map (D38) and the `McpServerError` is rendered as a **diagnostic only**; and `resolve_mcp_exit` intercepts the unsignalled `Transport{ConnectionClosed(_)}` pre-handshake disconnect (`is_pre_handshake_disconnect()`) BEFORE this `InternalError` cast, delegating the exit code to the teardown → exit 0 on a clean shutdown (D40). See §5b (`mcp`). Since **D50** that bounded render is DEFENCE-IN-DEPTH on the `ExpectedInitializeRequest` arms, which the pre-handshake gate makes unreachable from the wire; the `TransportError` arm and the wildcard stay live, and nothing in the D49 grammar or in this mapping moves.
...
Any `EngineError` → `StructuredError` (§2.4) attached as rmcp tool error **data** (`code`/`message`/`hint`/`retryable`/`context`), parallel to the CLI 0–8 exit codes. A failed tool call still returns **valid JSON** (the shared in-band error output — `SchemaBundle.error`, `is_error=true`; §5.3/§5.4 D25). **Argument-boundary contract (NORMATIVE, D42).** `schemars` is **codegen-only** — it publishes the `inputSchema` and there is **no runtime validator anywhere in the process**. Enforcement is **three** explicit steps: **(0) a byte-level DUPLICATE-KEY SCAN inside an OWNED `Transport<RoleServer>` (D43), BEFORE `serde_json::from_slice`** — a duplicated key is collapsed last-wins while the frame is decoded, so no layer downstream of the parse can observe it, and neither a `Transport` DECORATOR (`receive()` hands it an already-parsed message) nor the argument extractor (which receives an already-deduplicated `JsonObject`) is a possible home. The verdict is carried in `rmcp::model::Extensions`, which is **wire-unforgeable**: it has no `Serialize`/`Deserialize` impl at all, so no wire field can name it. **The scan is scoped to the WHOLE `params` value of every decoded request, `_meta` INCLUDED — NOT to `params.arguments` alone.** The scan itself is UNIVERSAL (`resources/*`, `prompts/*` included — it runs on the raw bytes before rmcp classifies the method), but the **verdict is consulted at exactly ONE site, `call_tool`**, because only `tools/call` has an in-band channel; gating another method would have to answer out-of-band and reopen the `-32602` arm this contract exists to keep shut. Non-`tools/call` methods therefore carry a stamped-but-unenforced verdict. **The verdict rule is FAIL-CLOSED: absent OR indeterminate ⇒ REJECT.** An empty `Extensions` is the DEFAULT state, so encoding "absent ⇒ clean" would make any path reaching a handler without traversing the scanning transport fail OPEN. The one documented verdict-less path is TEST-ONLY: `mcp_server_duplex_unclamped_for_test`, the CD-6 raw-rmcp pin, which deliberately installs no scan and whose calls the fail-closed arm refuses. **(1)** `enforce_quota` over the whole `tools/call` `params`, once, pre-dispatch, inside the rate-limit permit; **(2)** a typed parse under `#[serde(deny_unknown_fields)]` inside the tool body, reached via a crate-local DEFERRING `Parameters<T>` extractor whose `FromContextPart` impl is infallible — which is what makes rmcp's out-of-band `invalid_params` arm structurally unreachable for our 8 tools. Oversized, unknown-field and malformed **arguments** are therefore rejected **in-band** (`isError:true` + the FR-11 `StructuredError`, `retryable: true`) before reaching the engine; blast radius confined to the workspace. **A duplicate JSON key inside `params` (anywhere, `_meta` included) is NOT a member of the scoped list below** — it is rejected IN-BAND by step (0)/`call_tool` (D43). Note also that duplicated ENVELOPE fields are not caught by serde duplicate-field detection, contrary to a natural reading: `params.name`/`params.arguments` are reached through rmcp's `#[serde(flatten)]`, so the typed variant merely FAILS and the untagged fallback wins, yielding `CustomRequest` → `-32601`. **The rest of the envelope does NOT reduce to one outcome, and this enumeration is deliberately OPEN rather than closed — an earlier closed-list spelling of it omitted the `id` member entirely and stated the `params` case absolutely, and both were wrong (D47).** Member by member, measured: a duplicated `jsonrpc` or `method` hard-fails to `-32700` with the **id OMITTED**, so a diagnostic reaches the connection but an rmcp client — which DROPS an id-less error (`src/service.rs:1030-1036`) while awaiting untimed (`:442-447`, `:344-346`) — stays pending; **that is a DISCLOSED residual D47 deliberately leaves OPEN, tracked as `ub-788`**, and it is not covered by the ub-cnv criterion, since rmcp answers those frames too. A duplicated `params` is **METHOD-DEPENDENT**, not absolute: `-32700` on `tools/call` and `tools/list`, but a plain SUCCESS on `ping`, whose typed request takes no `params` at all. A duplicated `params._meta` (the KEY itself) hard-fails to `-32700`. A duplicate inside an UNKNOWN envelope member EXECUTES the tool normally with `isError:false` — shipped behaviour, pinned by `crates/unblock-cli/tests/duplicate_key_frames.rs::ns5_a_duplicate_outside_params_is_not_flagged`, and correct: the scan root is `params` and a sibling member is outside the contract. **And a duplicated — or otherwise UN-DECODABLE — envelope `id` is its own class, closed by D47 (PRD section 4):** rmcp's untagged union falls through to the Notification variant whenever `id` fails to decode, so the frame got NO reply at all until D47 answered it OUT-OF-BAND with `-32600 Invalid Request` on the id recovered from the raw bytes — or with the id omitted when those bytes are ambiguous or the value is no representable `RequestId` — and DROPPED it rather than delivering it. That `-32600` is a fault about the ENVELOPE, decided before any method is known, so it does not reopen the `-32602` argument arm this contract exists to keep shut, and D43's in-band `tools/call` channel is unchanged. **SCOPED — these stay out-of-band protocol faults. Three of them because NO seam under our control reaches them; the fourth, added by D47, because it is decided BEFORE any method is known and its frame never reaches the one seam that has an in-band channel:** an unknown tool name, a **non-object `arguments`** (rmcp fails to deserialize `CallToolRequestParams` itself, so `call_tool` is never entered), a present **`params.task`** (rejected by rmcp's default `TaskSupport::Forbidden` before `call_tool`), and — **D47** — an **UN-DECODABLE ENVELOPE `id`**, which OUR OWN transport (`crates/unblock-mcp/src/wire.rs`) answers `-32600 Invalid Request` on the id recovered from the raw bytes (omitted when they are ambiguous) and then DROPS. The blanket reason no longer holds for the whole list and is therefore stated MEMBER BY MEMBER: D47's frame decoded to a `Notification`, so `call_tool` — the ONE site with an in-band channel — is structurally unreachable for it, and the fault is about the ENVELOPE rather than the arguments, so the `-32602` argument arm this contract exists to keep shut stays shut. `ErrorData` remains reserved for exactly the class this enumeration NAMES; the list is the rule and it carries no count. **And a frame arriving BEFORE the handshake completes is its own class, closed by D50 (PRD section 4).** Every inbound frame rmcp read before it accepted an `initialize` Request, other than an `initialize` or a `ping`, returned `ExpectedInitializeRequest` and killed the server, so the class had no boundary behaviour at all. A decorator we own, sitting between the version clamp and the duplicate-key scan, now passes only `initialize` and `ping` upward before the handshake completes, matched by the `ClientRequest::InitializeRequest(_)` and `ClientRequest::PingRequest(_)` variants and never by `ClientRequest::method()`. A premature Request is answered OUT-OF-BAND `-32600 Invalid Request` on its own id and DROPPED; a premature Notification, Response or Error frame is dropped with no reply, because JSON-RPC 2.0 section 4.1 gives none of them one. The gate OPENS when the server's own `InitializeResult` passes through the decorator's `send()`, never on `notifications/initialized`, because rmcp serves a request arriving before that notification normally and this transport must not be stricter. That `-32600` is a fault about the PHASE, decided before any method is dispatched and before `call_tool` exists as a destination, so it does not reopen the `-32602` argument arm this contract exists to keep shut and D43's in-band `tools/call` channel is unchanged.
...
- The CLI maps each `EngineError`/boundary error → `StructuredError` and exits with its 0–8 exit code (§2.4); structured output to stdout, diagnostics to stderr (NFR-14) — **EXCEPT a command that owns stdout as a protocol channel, whose structured error goes to STDERR too (D48; `unblock mcp` today). The 0–8 code is unaffected by that choice.**
