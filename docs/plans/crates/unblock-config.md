...
|---|---|---|---|---|
| `Cargo.toml` | v1 (edit v1.1/v1.2) | Crate manifest. Deps: `unblock-storage`, `unblock-sync`, `unblock-health`, `unblock-error`, `serde`, `toml`, `snafu`, `tracing`, `directories` (XDG, v1.1). `[features] remote` (v1.2) gates `keyring`/remote types — off by default (D15/NFR-10). dev-deps: `proptest`, `insta`, `tempfile`. | `[features] default=[]`, `remote=["dep:keyring"]`; workspace lints inherited. | `cargo-deny` confirms no TLS/keychain on default tree (NFR-10). |
...
| `src/error.rs` | v1 (extend v1.1/v1.2) | Per-crate snafu enum + `code()` mapping (spine §2.1 pattern). | `#[derive(Snafu)] pub enum ConfigError { NotFound{}, Parse{source, path}, Io{source, path}, InvalidValue{key, value, reason}, DiscoveryFailed{start}, UnknownKey{key} /*warn-only path*/, /*v1.2*/ CredentialMissing{}, RemotePathNotConfined{} }`; `impl ConfigError { pub fn code(&self)->ErrorCode; pub fn hint(&self)->Option<String>; }` → `ConfigError`/`ConfigNotFound`/`ConfigParseError` (exit 7), I/O→`IoError` (exit 8), `RemotePathNotConfined`→`PathTraversal`(exit 6, v1.2). | unit: every variant maps to the expected `ErrorCode` + `exit_code()`; `insta` golden snapshot of `(variant → code → exit)` table (parallels spine §2.3, FR-11). |
...
| `src/context.rs` | v1 (storage open) | `ResolvedContext` (resolve-only, **no storage**) + `WorkspaceContext` (storage-bearing, CF-D). The two-facade split (G-5 option b / G-9): `ResolvedContext` bundles resolved config + paths + `workspace_dir` + `actor`; `WorkspaceContext` adds the **`Arc<dyn Storage>` this crate builds** as a **NON-OPTIONAL** field (spine §4.1 shape). Holds the `open_workspace`/`open_with_storage` entrypoints. Bridges to `unblock-storage` (opens + migrates libsql via the `Storage` constructor) and to `unblock-health`/`unblock-sync` only for path/preflight, never engine. | `pub struct ResolvedContext { config:WorkspaceConfig, paths:ConfigPaths, workspace_dir:PathBuf, actor:String }`; `pub struct WorkspaceContext { config:WorkspaceConfig, paths:ConfigPaths, workspace_dir:PathBuf, actor:String, storage:Arc<dyn Storage> /* NON-OPTIONAL — Session::open never unwraps an Option (G-5) */ }`; `pub fn open_workspace(cli:&CliOverrides)->Result<ResolvedContext,ConfigError>` (resolve only, no DB); `pub async fn open_with_storage(cli:&CliOverrides)->Result<WorkspaceContext,ConfigError>` (opens + migrates-if-needed libsql and **constructs the `Arc<dyn Storage>`**); accessors `config()`, `paths()`, `workspace_dir()`, `actor()` on both, plus `storage()` on `WorkspaceContext`. **Engine consumes the storage-bearing `WorkspaceContext` via `Session::open(ctx, cfg)`; config builds storage, engine never does (CF-D).** | integration (`tests/open_workspace.rs`): full discover→merge→resolve on a `tempfile` `.unblock/` (returns `ResolvedContext`); `open_with_storage` opens a real libsql file + `integrity_check` ok and yields a `WorkspaceContext` with a live `storage`; error path when dir missing. |
...
4. **Where does the libsql open live?** **RESOLVED (CF-D).** Spine §4 intro + §4.1 pin `unblock-config` as the owner of workspace-open: this crate performs `.unblock/` discovery, opens/migrates libsql, and **builds the `Arc<dyn Storage>`**, exposing a `WorkspaceContext { storage, workspace_dir, actor, resolved config }`. The engine **consumes** it via `Session::open(ctx, cfg)` and does not construct storage. `open_with_storage` (§3 `context.rs`) is the canonical entrypoint.
