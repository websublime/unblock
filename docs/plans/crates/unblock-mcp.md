...
- **OQ-1 (rmcp 1.7 API surface, RK-2):** the spine assumes `#[tool_router]`/`#[tool]` macro ergonomics and an `unblock://` resource-template API. Confirm rmcp 1.7's exact attribute-macro names, the `ServerHandler` trait shape, and how resource templates (`{id}`) are declared — pin the version and isolate any churn here. Does rmcp 1.7 give a built-in request-size limit, or must `Quotas` wrap the transport read?
- **OQ-2 (in-band vs. protocol error):** spine §5.6 says a failed tool call returns **valid JSON** (`ToolOutput::Error`) AND attaches rmcp tool-error `data`. Confirm rmcp 1.7 lets a tool both return content and signal `isError` — or do we choose one channel? (Default plan: return `ToolOutput::Error` content with `isError=true` + structured `data`.)
