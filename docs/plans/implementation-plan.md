...
- **T0.1 — Workspace scaffold deps.** Update root `Cargo.toml`: add `libsql` (features: local/bundled default; **remote/replica behind a non-default feature**, D15), `clap` (stable features only); swap `backoff` → `backon` (or `tokio-retry`); change `unsafe_code` `deny` → `forbid`; confirm `resolver = "2"`, workspace lints. *(No crates yet; just the workspace manifest + `[workspace.dependencies]`.)* **AC: the empty workspace `cargo build`s on stable 1.96 with `libsql`+`rmcp`+`clap`+`axoupdater`+`backon` resolved together — the first real check that the pinned versions co-exist.**
- **T0.2 — Create empty crates with layering.** `cargo new` the 12 `unblock-*` crates as workspace members with the dependency edges from PRD §8.1; add the layering CI check. *(This is the "create empty crates" step Miguel deferred — first executed task after plan approval.)*
...
- **T0.9 — CI scaffolding (M0).** Author `.github/workflows/ci.yml` (fmt / clippy-pedantic / `cargo test` / `cargo-audit` / `cargo-deny` / `insta --check` / layering check / **doc-lint** per ci-cd §2.1), `deny.toml` (licenses + no-git ban + transitive budget), `.cargo/config.toml`, and confirm `rust-toolchain.toml` (1.96). All `uses:` SHA-pinned (NFR-9). Depends on T0.2. *(AC: CI green on the empty workspace; the doc-lint runs and passes.)*
...
- **T1.3 — `unblock-config` (v1 subset).** Layered TOML resolution **CLI > env (`UNBLOCK_*`) > project `.unblock/config.toml` > defaults**; startup-vs-runtime partitioning; `.unblock/` discovery; open-a-workspace facade. (DB-config-table + user-config layers = v1.1.) *(AC: precedence unit tests, FR-13 subset.)*
- **T1.4 — Issue lifecycle (FR-1a/1b/1c, FR-2, FR-3) via engine.** create/quick-create; show/update(multi-id, labels set/add/remove, reparent w/ cycle reject); delete (tombstone/cascade/hard/dry-run); atomic `claim`; defer/undefer. *(AC: each FR's AC; claim race test via the contention lab.)*
...
- **T1.6 — Dependencies & graph (FR-5) via engine.** typed edges; add/remove/list/tree/cycles; `petgraph` traversal + cycle detection (reject `blocks` cycles with the cycle path). *(AC: edge change reflected in ready immediately.)*
...
- **T2.2 — `unblock-mcp`: rmcp stdio server.** `unblock serve` on `rmcp 1.0` (`server`, `transport-io`); `#[tool_router]`; capabilities (tools/resources/prompts); errors → MCP error data mirroring exit codes (FR-11/FR-20). Thin adapter over `unblock-engine`.
- **T2.3 — MCP tool/resource/prompt taxonomy (closes PRD §12.2).** Implement the consolidated v1 surface in §6 below. *(AC: tool count ≤ target; args `schemars`-validated with size/rate limits, NFR-18.)*
...
- **T3.5 — Performance budgets (NFR-1/NFR-2).** `criterion` baselines + 10% regression gate; 250k-issue CI corpus under the single-serve topology; record agent round-trip latency (§14 metric).
...
M0 → T1.1 policy → T1.2 engine → {T1.3 config, T1.4 lifecycle, T1.5 query, T1.6 deps}   [M1 GATE]
