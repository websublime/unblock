...
**Status:** APPROVED (`rbac.For` read-path explicit-column projection applied 2026-06-17 — §10.1 `ScopedQuery` gains an optional `Columns(...)` projection option + §3.4 records the binary-`tsvector`/OID-3614 runtime fact + §10.1.1/§11.3 lockstep, so the `workitems.items` reads exclude the unscannable generated `fts` column instead of emitting `SELECT *`, closing the `exitcriteriontest` `scan tsvector (OID 3614)` runtime failure; phantom `itemRow.FTS` removed; default `SELECT *` preserved for all other consumers; no DDL/root-SPEC change, bead `unblock-8xb.8`; cascade publish-trigger / §5.7.1 derivation-input reconciliation applied 2026-06-15 — §6.2 Tool 13 `set_state` + §6.2 Tool 10 `comment` Side-effects, §6.3.0 Regime B + `state_change` row + non-publisher list, §6.3.2 subscriber + enum bullet, §4.4 `SetStateColumns` + `AppendComment` doc-comments reconciled so the cascade-publish trigger set EQUALS the AUTHORITATIVE root-SPEC §5.7.1 derivation-input set: a pure `pipeline_state` write AND an investigation/review comment append are §5.7.1-affecting and MUST publish `CascadeRequested{state_change}` — closing the STALE-`pipeline_stage` internal self-contradiction (the former "pure `pipe_state` is exempt" carve-out, FACTUALLY WRONG vs §5.7.1 rows 1–3, RETIRED); no DDL/migration/derivation-table change, root SPEC untouched (bead `unblock-tv8.87`); org-provisioning write-surface tenant gates applied 2026-06-15 — §10.1.1 gate-model + §4.2 extended to the two `org` RPCs that write tenant-scoped rows from the wire (`org.AddMember` caller-admin `org.members` predicate + role cap — closing a CRITICAL cross-tenant privilege escalation; `org.CreateProject` caller-membership predicate replacing the FK→`NotFound` that only caught a non-existent org — closing a WARNING write IDOR), the sibling org-provisioning RPCs the `.85` admin/BFF sweep set aside — DORMANT (empty-caller no-op) until a future key-management BFF pins the caller identity (bead `unblock-tv8.86`); auth/BFF admin write-surface tenant gates applied 2026-06-15 — §10.1.1 gate-model extended to the two `auth` RPCs that write `mcp.api_keys` (`IssueAPIKey` `org.Authorize`-on-`CallerUserID` ownership + `issued_to_user ∈ org.members`; `RevokeAPIKey` `CallerOrgID` row predicate on the UPDATE), closing TWO LATENT cross-tenant write IDORs on the admin/BFF surface the MCP-wire sweep set aside — DORMANT (empty-caller no-op) until a future key-management BFF pins the caller identity (bead `unblock-tv8.85`); `Update.milestone_id` write-scope tenant gate applied 2026-06-15 — §10.1.1 gate-model extended to `workitems.Update`'s wire-supplied `milestone_id` selector (org-XOR-project milestone predicate, `AssignItem`/`Create` precedent), closing the CRITICAL cross-tenant write IDOR that `.83`'s AC4 wrongly assumed gated (bead `unblock-tv8.84`); `create_milestone.project_id` INSERT-scope tenant gate applied 2026-06-15 — §10.1.1 gate-model extended to `workitems.CreateMilestone`'s project-scoped `project_id` selector (guarded INSERT…SELECT, `CreateLabel` precedent), closing the last ungated cross-reference write-IDOR in the family (bead `unblock-tv8.83`); create-path cross-reference tenant validation applied 2026-06-12 — §10.1.1 gate-model extended to `workitems.Create`'s wire references, keyed on the existing `req.OrgID` per Miguel's 2026-06-12 DECISION, + §4.4 `CreateRequest`/`DependencyEdge` drift reconciliation; round-16 write-surface tenant-hardening lockstep applied 2026-06-11 — §10.1.1 row-level write gate + per-RPC `CallerOrgID` channel + deps fold-in + `CreateLabel` empty-`CallerOrgID` guard; round-16 label-tools drift-closure applied 2026-06-11 — labels `updated_at` migration `0130` + Tools 20–23 auth wording + Tools 16/19 wire-sample/prose alignment; round-16 P01 tool-surface scope amendment applied 2026-06-04; §3.5 fifth-secret addition applied 2026-06-02; round-15 NFR-1 harness test-isolation + mcpaudittest hardening applied 2026-05-29; round-14 NFR-1 latency-harness scope applied 2026-05-29; round-6 cascade-symmetry applied 2026-05-12; round-5 tracing applied 2026-05-12; round-4 auth applied 2026-05-11; DRIFT-1/-2 applied 2026-05-08; round-2 applied 2026-05-08; round-3 research applied 2026-05-08; original APPROVED 2026-05-07)
**Changelog:**
- 2026-06-17 — cosmetic label correction (bead `unblock-8xb.8`; status remains APPROVED): the `rbac.For[itemRow]` reflection-scan read-RPC call-site list is corrected from "Get / GetTrail / List / Search" to the accurate FIVE sites "Get / GetState / GetTrail / List / Ready" (confirmed against `apps/api/workitems/workitems.go` — `Search` is a direct UNION-ALL SQL query that does NOT route through `rbac.For`, and `GetState`/`Ready` were omitted); no prose/behaviour change.
- 2026-06-17 — round-18 `tsvector`-projection completeness fix to round-17 (bead `unblock-8xb.8`; status remains APPROVED — this CORRECTS an over-narrow scope claim in the round-17 entry below and ADDS a forward constraint, a bug-fix not a re-architecting). **What round-17 got wrong (confirmed against code):** the round-17 entry claimed "default `SELECT *` is preserved for every `rbactest` matrix consumer (no call-site change) — they are safe." An exhaustive sweep of EVERY `rbac.For[T]` call site vs EVERY `tsvector` column DISPROVES that for TWO `rbactest` consumers. THREE tables carry a `tsvector` column (the only Postgres type with no binary scan-plan into a Go scalar under the Encore pgx v5.7.6 runtime, OID 3614): `workitems.items.fts` (`0040_workitems.up.sql:151`), `workitems.comments.fts` (`0040:231`, no `rbac.For` read yet), and `memory.entries.ts_doc tsvector NOT NULL` (`0090_memory.up.sql:29` — round-17 NEVER mentioned this table). The `rbactest` read `rbac.For[workitemsItemsRow](id,"workitems.items")` at `rbactest_test.go:589` (phantom `workitemsItemsRow.FTS []byte`, `rbactest_test.go:456`) and `rbac.For[memoryEntriesRow](id,"memory.entries")` at `rbactest_test.go:633` (phantom `memoryEntriesRow.TSDoc []byte` at field position 11, `rbactest_test.go:537`) BOTH project a `tsvector` and therefore FAIL `rows.Scan` on a populated result set on the Encore platform (local passes only because the emulator delivers text wire format; the platform delivers binary OID 3614 — same latent mode as the round-17 production `workitems.items` defect). The round-17 scope note was too narrow because it overlooked `memory.entries.ts_doc` and the `rbactest` `workitems.items` read. **The fix:** the two `rbactest` reads (:589, :633) MUST pass an explicit `.Columns(...)` projection excluding the `tsvector` column (`items.fts` resp. `entries.ts_doc`), and the phantom `FTS` / `TSDoc` fields are REMOVED — same shape as the five production `workitems.items` reads round-17 fixed. The OTHER six `rbactest` reads (`org.projects` :567, `org.members` :578, `deps.cascade_events` :600, `mcp.tool_calls` :611, `mcp.api_keys` :622, `boards.boards` :648) and the production `org.go:455` read target `tsvector`-FREE tables (jsonb/bytea scans into `[]byte` work in binary; only `tsvector` lacks the plan) and correctly KEEP default `SELECT *` — NOT touched. **General rule pinned:** a `rbac.For[T]` read of ANY `tsvector`-carrying table MUST exclude that column via `.Columns(...)`. **Forward constraint (production `memory.entries`):** the memory service is P02 Track B (beads `unblock-8xb.2.x`) and not yet built — it has NO production exposure today. When its reads of `memory.entries` go through `rbac.For[T]`, they MUST use `.Columns(...)` excluding `ts_doc`, or they reintroduce this exact bug in production. This constraint is placed where the Track B implementer reads it — `02-spec-backend-complete.md` §5.6 (memory-service RBAC) — and cross-referenced from §3.4/§10.1 here. **Patched in lockstep (docs only):** §3.4 FTS note (corrected round-17 "rbactest safe" claim + the three-table general `tsvector` rule + the two unsafe `rbactest` reads + the six safe ones + the `org.go:455` exclusion); §10.1 projection-rationale prose (round-18 correction subsection); §10.1.1 read-path intro (parenthetical broadened from `workitems.items`-only to the general `tsvector`-table rule); `02-spec-backend-complete.md` §5.6 (NEW production `memory.entries` `.Columns`-excluding-`ts_doc` forward constraint). **NOT touched (separate ownership):** `apps/api/shared/rbactest/rbactest_test.go` (the :589 / :633 `.Columns(...)` additions + the `workitemsItemsRow.FTS` / `memoryEntriesRow.TSDoc` phantom-field removals) — the implementation bead `unblock-8xb.8` (Greta) owns those on its branch. **No DDL / migration / public-API change.** **Root `docs/SPEC.md` is UNTOUCHED** — its §5.6 RBAC prose is projection-agnostic (carries no `SELECT *` claim), so it asserts nothing now false; the `.Columns`-excludes-`tsvector` rule STRENGTHENS it. Spec status remains APPROVED.
- 2026-06-17 — `rbac.For` read-path explicit-column projection (`Columns(...)`) closing the `workitems.items` `fts tsvector` scan failure (bead `unblock-8xb.8`, surfaced by the Encore-platform full test run — `apps/api/exitcriteriontest` `TestExitCriterion_AuthBearerResolvesIdentity` (Ready) + `TestExitCriterion_CycleDetected` (Get) fail with `rbac: scan "workitems.items": can't scan into dest[28] (col: fts): cannot scan tsvector (OID 3614) in binary format into *[]uint8`; status remains APPROVED — this is an additive extension of the §10.1 `rbac.For`/`ScopedQuery` contract + the removal of a phantom row field, NOT a re-architecting). **Root cause (confirmed against code):** `rbac.For[T](id, table).build()` emits `SELECT * FROM <table>` (`apps/api/shared/rbac/rbac.go:431`) and `scanAll[T]` scans by struct-field ordinal via reflection (`rbac.go:512`); `SELECT *` projects the generated `fts tsvector` column (migration `0040_workitems.up.sql:151-156`, ordinal 28). `apps/api/workitems/helpers.go:45-82` carried a phantom trailing `itemRow.FTS []byte` field purely to keep ordinals aligned, on the ASSUMPTION (helpers.go:74-81) that "pgx v5.7 has no registered tsvector type, so the driver delivers the text representation as a raw byte slice." **That assumption is FALSE under the Encore runtime:** pgx v5.7.6 returns `fts` in BINARY format and there is NO scan-plan from binary tsvector (OID 3614) → `[]byte`, so `rows.Scan` fails on any POPULATED result set (empty sets pass — why the latent P01 defect only now blocks the P02 exit criterion). Affects every `rbac.For[itemRow]` read that returns ≥1 row (`workitems.go:1289,1345,1416,2442,2626` — Get/GetState/GetTrail/List/Ready). pgx v5.7.6 + encore.dev v1.52.1 sources contain NO tsvector/OID-3614 registration, so any fix that scans the tsvector VALUE into a Go type is fragile; the robust fix is to STOP PROJECTING `fts`. **Locked contract (option A, decided by Miguel — fix now via spec-first):** extend the `ScopedQuery[T]` surface with an OPTIONAL `Columns(cols ...string) *ScopedQuery[T]` fluent option; `build()` defaults to `SELECT *` when `Columns` is never called (so `org.projects` and every `rbactest` matrix consumer that maps T 1:1 to the full table keep working with ZERO call-site change — the "default behavior must not silently break them" constraint), and emits an EXPLICIT ordered projection when `Columns` is set. The `workitems.items` read RPCs pass the canonical fts-excluding `itemColumnList` (`helpers.go:33-38`, which already deliberately excludes `fts`) via `.Columns(...)`, and the phantom `itemRow.FTS []byte` field is REMOVED. This is precisely the `Columns([]string)` hook the original `Run` doc-comment (`rbac.go:378-381`) anticipated. The scope-predicate injection (`<table>.org_id = $1`) and the no-compile-without-scope (`ErrMissingScope`) guarantees are ORTHOGONAL to the projection and PRESERVED unchanged. SECURITY: column identifiers are SQL identifiers with no bind channel, so each MUST be a Go string literal or untyped string constant — the §11.3 / `no_rbac_dynamic_clause.go` analyzer is extended to reject a non-literal `Columns` argument, the same constraint already on `For`'s `table` (unblock-tv8.35) and `Where`'s `clause` (unblock-tv8.33). **Patched in lockstep (docs only):** §10.1 `ScopedQuery` Go surface (new `Columns` method + the round-17 projection-rationale prose justifying explicit-`Columns` over field-derived or mandatory-list shapes); §3.4 FTS DDL (new "`fts` never projected on the `rbac.For` read path" note recording the binary-tsvector / OID-3614 runtime fact + the `workitems.comments.fts` symmetric caution); §10.1.1 read-path intro (parenthetical noting the `workitems.items` reads pass the fts-excluding projection, scope unchanged); §11.3 RBAC-literal invariant (extended to cover the `Columns` variadic args as a third SQL-identifier sink + `unblock-8xb.8` provenance). **NOT touched (separate ownership): `apps/api/shared/rbac/rbac.go` (the `Columns` method + `build()` projection branch + the `Run`/`scanAll` doc-comments), `apps/api/workitems/helpers.go` (remove `itemRow.FTS`; the `itemColumnList` sync comment), `apps/api/workitems/workitems.go:1289,1345,1416,2442,2626` (add `.Columns(itemColumnList)` to the five `rbac.For[itemRow]` reads), `apps/api/shared/lint/no_rbac_dynamic_clause.go` + its testdata (extend to the `Columns` call shape), and the tests — the implementation bead `unblock-8xb.8` (Greta) owns those on its branch.** **No DDL / migration / public-API change** — the gate is a SELECT-projection change + a Go surface addition; the `workitems` schema (incl. the `fts` generated column + GIN index, still used query-side by `search`) is UNCHANGED. **Root `docs/SPEC.md` is UNTOUCHED** — its §5.6 RBAC prose (lines 776-785) describes "row-level filtering … a typed query builder that refuses to compile a query missing the scope filter" at an abstraction ABOVE the SELECT-projection shape; it carries NO `SELECT *` claim, so the explicit-`Columns` projection STRENGTHENS (does not contradict) it. The `02-spec-backend-complete.md` `rbac.For` references (lines 796-797, 1025-1033) cite only the `<table>.org_id = $1` SCOPE predicate (preserved), not the projection; the `docs/plans/*` carry no `SELECT *` claim — all three left untouched (verified by grep). Spec status remains APPROVED.
...
**`fts` is never projected on the `rbac.For` read path (round-17, bead
`unblock-8xb.8`).** The generated `fts tsvector` column is delivered by
the Encore pgx v5.7.6 / encore.dev v1.52.1 runtime in BINARY wire format
(OID 3614) with NO registered scan-plan into any Go scalar (`[]byte`
included) — neither pgx nor encore.dev registers a tsvector type. The
`rbac.For[itemRow]` read RPCs (Get / GetState / GetTrail / List / Ready) therefore
MUST NOT emit `SELECT *` (which projects `fts`); they pass the canonical
fts-excluding `itemColumnList` projection via `ScopedQuery.Columns(...)`
(§10.1). `fts` is query-side only (the `search` `@@`/GIN predicate); no
read path scans its value into Go. `workitems.comments.fts` is identical
in nature — any future `rbac.For[commentRow]` read MUST exclude it the
same way.

**General rule (round-18, bead `unblock-8xb.8`): a `rbac.For[T]` read of
ANY table carrying a `tsvector` column MUST exclude that column via
`.Columns(...)`.** Three tables in the schema carry a generated/declared
`tsvector` column — the only Postgres type with no binary scan-plan into
a Go scalar under the Encore pgx v5.7.6 runtime (OID 3614): (1)
`workitems.items.fts` (`0040_workitems.up.sql:151`), (2)
`workitems.comments.fts` (`0040:231` — currently no `rbac.For` read over
comments), and (3) `memory.entries.ts_doc tsvector NOT NULL`
(`0090_memory.up.sql:29`). The round-17 scope note claimed "default
`SELECT *` is preserved for the `rbactest` matrix consumers (no call-site
change) — they are safe." **That claim was too narrow and is CORRECTED
here:** it overlooked `memory.entries.ts_doc` (round-17 never mentioned
the `memory.entries` table) and the `rbactest` `workitems.items` read.
TWO `rbactest` consumers DO project a `tsvector` and therefore are NOT
safe on default `SELECT *` against the Encore platform (local passes only
because the local emulator delivers text wire format; the platform
delivers binary OID 3614): the `rbac.For[workitemsItemsRow]` read at
`rbactest_test.go:589` (its `workitemsItemsRow.FTS []byte` phantom field
is `items.fts`) and the `rbac.For[memoryEntriesRow]` read at
`rbactest_test.go:633` (its `memoryEntriesRow.TSDoc []byte` phantom field
at position 11 is `entries.ts_doc`). Both MUST pass an explicit
`.Columns(...)` projection that excludes the `tsvector` column, and the
phantom `FTS` / `TSDoc` fields are REMOVED — same fix shape as the five
production `workitems.items` reads. The OTHER six `rbactest` reads
(`org.projects` :567, `org.members` :578, `deps.cascade_events` :600,
`mcp.tool_calls` :611, `mcp.api_keys` :622, `boards.boards` :648) and the
production `org.go:455` read target `tsvector`-FREE tables — jsonb/bytea
scan into `[]byte` works in binary format, only `tsvector` lacks the
plan — so they correctly KEEP default `SELECT *` and are NOT touched. The
production `memory.entries` reads do not exist yet (the `memory` service
is P02 Track B, beads `unblock-8xb.2.x`); the forward constraint for them
lives in `02-spec-backend-complete.md` §5.6 (see §10.1 cross-reference).
...
`rbac.For` read RPCs (Get / GetState / GetTrail / List / Ready), `MilestoneTree`
...
RPC. Unlike the `rbac.For` read RPCs (Get / GetState / GetTrail / List / Ready),
...
// ordinal. Columns is OPTIONAL: a ScopedQuery on which Columns is never
// called keeps the default `SELECT *` shape (so every consumer that maps
// T 1:1 to the full table — `org.projects`, the rbactest matrix rows —
// is UNAFFECTED). It exists for the `workitems.items` read path, whose
// table carries a generated `fts tsvector` column that has NO binary
// scan-plan into any Go type under the Encore pgx runtime (§3.4); the
// read RPCs pass the canonical fts-excluding projection so `SELECT *`
// never projects `fts`. SECURITY: each column identifier MUST be a Go
// string literal or untyped string constant — SQL identifiers have no
// bind channel (same §10.1 / unblock-tv8.35 constraint as `table`); the
// §11.3 analyzer rejects a non-literal Columns argument.
...
**Projection: explicit `Columns(...)` over `SELECT *` (round-17, bead
`unblock-8xb.8`).** `build()` defaults to `SELECT * FROM <table>`. For
`workitems.items` this projected the generated `fts tsvector` column
(migration `0040_workitems.up.sql:151`), which the Encore pgx v5.7.6 /
encore.dev v1.52.1 runtime delivers in BINARY format with NO registered
scan-plan into `[]byte` (OID 3614) — so `scanAll`'s ordinal scan fails at
runtime on any populated result set (`can't scan into dest[N] (col: fts):
cannot scan tsvector (OID 3614) in binary format into *[]uint8`). The
former accommodation — a phantom trailing `itemRow.FTS []byte` field to
keep ordinals aligned — assumed pgx would deliver the tsvector as its
TEXT representation; that assumption is FALSE under the Encore runtime.
The robust fix is to STOP PROJECTING `fts` on the read path: the
`workitems.items` read RPCs (Get / GetState / GetTrail / List / Ready) pass the
canonical fts-excluding `itemColumnList` projection via `.Columns(...)`,
and the phantom `itemRow.FTS` field is removed. `.Columns` is the
explicit-`Columns([]string)` hook the original `Run` doc-comment
anticipated ("future phases may add an explicit Columns([]string) hook
without breaking the locked surface"); it is OPTIONAL and additive, so
the `rbac.For` consumers that scan a `tsvector`-FREE table keep the
default `SELECT *` shape with NO call-site change. The scope-predicate
injection and the no-compile-without-scope guarantee are orthogonal to
the projection and are PRESERVED unchanged.

**Round-18 correction (bead `unblock-8xb.8`).** Round-17 stated the
default `SELECT *` shape is preserved for "every `rbactest` matrix
consumer" with no call-site change. That was too narrow: TWO `rbactest`
consumers project a `tsvector` column and therefore are NOT safe on
default `SELECT *` against the Encore platform — `rbac.For[workitemsItemsRow]`
(`workitems.items`, `rbactest_test.go:589`) and `rbac.For[memoryEntriesRow]`
(`memory.entries`, `rbactest_test.go:633`). Both MUST pass an explicit
`.Columns(...)` projection excluding the `tsvector` column (`items.fts`
resp. `entries.ts_doc`), with their phantom `workitemsItemsRow.FTS` /
`memoryEntriesRow.TSDoc` fields removed — the same fix as the five
production `workitems.items` reads. The round-17 note overlooked
`memory.entries.ts_doc` (never mentioned that table) and the `rbactest`
`workitems.items` read. The OTHER six `rbactest` reads and the production
`org.go:455` read target `tsvector`-FREE tables and correctly stay on
default `SELECT *`. **The general rule (§3.4): a `rbac.For[T]` read of
ANY table carrying a `tsvector` column excludes that column via
`.Columns(...)`.** The forward constraint for the not-yet-built
production `memory.entries` reads (P02 Track B) lives in
`02-spec-backend-complete.md` §5.6.
...
The read path self-gates in SQL via `rbac.For[T]` (§10.1 above; reads of
any `tsvector`-carrying table additionally pass an explicit
`.Columns(...)` projection excluding the `tsvector` column per the
round-17/18 projection rule of §3.4 — the `workitems.items` reads exclude
`fts`; the scope predicate is unchanged). The
...
- [ ] **Single-writer invariants (round-6 §6.3.0 — fractured).** The
  ...
  - (b) **`is_ready` single-writer = the mutating call site
    ...
    this round.
- [ ] `rbac.For`, `rbac.ScopedQuery.Where`, and `rbac.ScopedQuery.Columns`
  are never called with runtime-constructed string arguments — every
  table identifier (For arg 2), every clause string (Where arg 1), AND
  every column identifier (Columns variadic args, round-17) MUST be a Go
  string literal or untyped string constant. Runtime values destined for
  Where flow through `args...` exclusively; the table and column
  identifiers have no runtime channel (SQL identifiers cannot be bound).
  (Static analysis: `golangci-lint` custom linter rule under
  `apps/api/shared/lint/no_rbac_dynamic_clause.go` rejects any
  non-literal call site across the unblock backend for ALL THREE call
  shapes. Locked SPEC §10.1 surface is unchanged; the analyzer is a
  meta-guard. unblock-tv8.33, unblock-tv8.35, unblock-8xb.8.)
