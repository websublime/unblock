...
the other `rbac.For` consumers that scan the full table (`org.projects`,
every `rbactest` matrix row) keep the default `SELECT *` shape with NO
call-site change. The scope-predicate injection and the
...
`workitems.items` reads additionally pass an explicit fts-excluding
`.Columns(...)` projection per the round-17 projection note — the scope
predicate is unchanged). The
