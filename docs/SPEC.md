...
- `verify_can_transition` is **not** a separate top-level MCP tool — it is a
  ...
  `Server.AddTool`. (Mechanism note, research R-P02-6/C2 2026-06-16: the
  `modelcontextprotocol/go-sdk` v1.6.0 has **no "registered-but-unlisted
...
-- Webhook-secret model (research R-P02-4/C1 2026-06-16; Q7 RESOLVED
-- 2026-06-16 — DROP). The confirmed v1.0 GitHub path is a **GitHub App**: a
-- single **app-level** webhook URL + **one app-level webhook secret** fires
-- for ALL installations, and each delivery carries `installation.id` in the
-- payload to disambiguate. HMAC verification must therefore run BEFORE the
-- body is parsed (to learn the installation), so under the App path the
-- secret is read from an **Encore application secret** (e.g.
-- `GITHUB_APP_WEBHOOK_SECRET`, provisioned in Track E-2) — NOT from
-- `installations.webhook_secret_enc`. Because the per-install column is dead
-- under the confirmed App path, it is **DROPPED via a NEW additive forward
-- migration** (a forward `ALTER TABLE providers.installations DROP COLUMN
-- webhook_secret_enc`). The P01 `0060_providers` migration is **NOT** edited
-- (`feedback_migration_edit_drift`); the drop ships as a new, higher-numbered
-- forward file (P02 spec authors the exact file). `providers.installations`
-- is an empty stub pre-production, so the drop is safe. The column is
-- **re-added by a future migration** when the OAuth-app / future GitLab
-- per-install fallback model ships (v1.1). The P02 spec pins the App-path
-- verification against the app-level Encore secret.
