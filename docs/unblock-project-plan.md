...
| Task | Description | DoD | Ref |
|---|---|---|---|
| **3.2.0** Build target validation | `git2` with `libgit2-sys` vendored chosen (validated 2026-04-01 on macOS ARM64). Add `git2` + `libgit2-sys` vendored to `unblock-core` deps. CI job that compiles and runs a blob round-trip test (write_blob → read_blob → create_ref → read_ref → list_refs → delete_ref) on all 5 distribution targets: Linux musl x86_64, Linux musl ARM64, macOS x86_64, macOS ARM64, Windows x86_64 | All 5 targets compile and pass the blob round-trip test in CI. Build time delta documented. Spike is a prerequisite blocker for all other 3.2 tasks and for 3.4 Distribution | Risk Register |
...
| **git2 build targets (Phase 3)** | **Medium — blocks Epic 3.2 and 3.4 Distribution** | **`git2` with `libgit2-sys` vendored chosen (validated 2026-04-01 on macOS ARM64). Remaining: CI validation on 5 distribution targets (Linux musl x86_64/ARM64, macOS x86_64/ARM64, Windows x86_64). musl + vendored libgit2 static linking has known edge cases. Task 3.2.0 adds CI job to confirm all targets compile and pass blob round-trip. Blocker for 3.2 implementation and 3.4 Distribution.** |
