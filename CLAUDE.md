...
| Doc | Role |
|---|---|
| `docs/PRD.md` | Product truth — decisions (§4 D1..D24), FR/NFR, domain model, milestones. **APPROVED v1.1.** |
...
- `unblock-engine` is the **single mutation home** (FR-9): MCP and CLI are thin adapters over `Session`, so they
  cannot drift. Writes serialize through an **in-process tokio `Semaphore`** (D14, single-serve per workspace).
- `unblock-cli` → `unblock-mcp` (cli owns the binary; mcp is a library exposing `serve()`); **never** mcp → cli.
...
Actions pinned to 40-char SHAs. Release via **`dist`** (6 target triples, shell+powershell installers, GitHub
