...
struct CorpusIndex {
    /// D-ids defined in PRD §4 (`| **Dx** |` rows), e.g. {1..=17}.
...
fn class_a_d_ids(doc: &Doc, guards: &Guards, index: &CorpusIndex, out: &mut Vec<Finding>) {
    // Spec tokenizes `\bD(1[0-7]|[1-9])\b` for the in-range ids (D1..D17), but an undefined ref
    // (D18, D99) is *also* a violation — so we match every `\bDn\b` and resolve membership against
