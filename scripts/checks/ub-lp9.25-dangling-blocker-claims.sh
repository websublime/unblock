...
# prose range `(D1..D45)` and the tokenizer's regex ALTERNATION `\bD(45|44|43|…)\b`. Pinning only the
# prose is exactly how that site rots into an undefined-D45 finding — the lint would stop tokenizing the
# id it is being told exists. Both halves are therefore separate Q rows on the same anchor.
RANGE_RE='D1\.\.D45'
RANGE_ALT_RE='D\(45\|44\|'
